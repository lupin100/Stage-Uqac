...
const fetchMembers = async () => {
    ...
    try {
        const response = await fetch(
            `${import.meta.env.VITE_API_URL}/api/persons/filter/${encodeURIComponent('Membre associé')}`
        )
...
<template>
    <v-container class="py-10" max-width="1200">
        <h2 class="text-h4 font-weight-bold mb-8 color-title">Membres associés</h2>
        ...
        <div v-else>
            ...
            <v-empty-state v-if="members.length === 0" icon="mdi-account-off-outline" title="Aucun membre trouvé"
                text="La base de données ne contient aucun membre associé pour le moment.">
            </v-empty-state>
        </div>
    </v-container>
</template>