                ...
                <v-card class="elevation-12 pa-6" rounded="xl">
                    ...
                    <v-card-text>
                        <v-form @submit.prevent="handleLogin">
                            <v-text-field v-model="username" label="Nom d'utilisateur" prepend-inner-icon="mdi-account"
                                variant="outlined" color="primary" class="mb-2" required></v-text-field>

                            <v-text-field v-model="password" label="Mot de passe" prepend-inner-icon="mdi-lock"
                                :append-inner-icon="isVisible ? 'mdi-eye-off' : 'mdi-eye'"
                                :type="isVisible ? 'text' : 'password'" variant="outlined" color="primary"
                                @click:append-inner="isVisible = !isVisible" required></v-text-field>
                            ...
                            <v-btn type="submit" color="primary" block size="large" class="mt-6 font-weight-bold"
                                :loading="isLoading" rounded="pill">
                                Se connecter
                            </v-btn>
                        </v-form>
                    </v-card-text>
